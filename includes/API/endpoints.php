...
function certificate_generator_register_api_routes() {
    ...
    register_rest_route('certificate-generator/v1', '/certificates-by-email', array(
        'methods'             => 'GET',
...
function certificate_generator_get_certs_by_email_cpt(string $email): WP_REST_Response {
    ...
    $student_query = new WP_Query(array(
        ...
        'meta_query'     => array(
            ...
            array('key' => 'student_email', 'value' => $email, 'compare' => '='),
        ),
    ));
    ...
    if (!empty($student_ids)) {
        $cert_query = new WP_Query(array(
            ...
            'meta_query'     => array(
                'relation' => 'OR',
                array('key' => 'student_id', 'value' => $student_ids, 'compare' => 'IN'),
                array('key' => 'student',    'value' => $student_ids, 'compare' => 'IN'),
            ),
            'orderby' => 'date',
            'order'   => 'DESC',
        ));
