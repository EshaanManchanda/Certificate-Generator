...
function cg_render_integration_dashboard_widget() {
	...
	<table class="cg-integration-links-table">
		...
		echo '<td style="color:#555;">';
		if ( $chatbot_dt_sync ) {
			echo $chatbot_dt_last ? esc_html__( 'Last: ', 'certificate-generator' ) . esc_html( date( 'd-m-Y H:i', $chatbot_dt_last ) ) : esc_html__( 'Never synced', 'certificate-generator' );
		} else {
			echo esc_html__( 'Requires both plugins active', 'certificate-generator' );
		}
		echo '</td></tr>';
	...
	if ( $dt_active && class_exists( 'WP_Dynamic_Tags_Table_Manager' ) ) {
		...
		$cert_tags     = $table_manager->get_tags(
			array(
				'search' => 'cg_',
				'limit'  => 0,
			)
		);
		$event_tags    = $table_manager->get_tags(
			array(
				'search' => 'event_',
				'limit'  => 0,
			)
		...
		printf(
			esc_html__( 'Cert Tags in Dynamic Tags: %1$d &bull; Event Tags: %2$d', 'certificate-generator' ),
			count( $cert_tags ),
			count( $event_tags )
		);
		echo '</p>';
