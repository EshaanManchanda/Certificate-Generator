...
// AJAX handlers for license server
add_action( 'wp_ajax_cg_save_license_server', 'cg_ajax_save_license_server' );
add_action( 'wp_ajax_cg_test_license_server', 'cg_ajax_test_license_server' );
add_action( 'wp_ajax_cg_verify_now', 'cg_ajax_verify_now' );

function certificate_generator_save_api_key_ajax() {
	if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'save_api_key_nonce' ) ) {
	...
	update_option( 'certificate_generator_api_key_enabled', true );
	wp_send_json_success( 'API key saved and API access enabled successfully' );
}

// AJAX handler for deleting API key
function certificate_generator_delete_api_key_ajax() {
	if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'delete_api_key_nonce' ) ) {
		wp_send_json_error( 'Invalid nonce' );
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_send_json_error( 'Insufficient permissions' );
...
function certificate_generator_email_logo_render() {
	...
	<div id="<?php echo esc_attr( $preview_id ); ?>" style="margin-top:10px;<?php echo empty( $email_logo ) ? 'display:none' : ''; ?>">
		<?php if ( ! empty( $email_logo ) ) : ?>
			<img src="<?php echo esc_url( $email_logo ); ?>" alt="Email Logo Preview"
...
function certificate_generator_smtp_settings_render() {
	...
	echo '<h3 style="margin-top:0">' . esc_html__( 'Test Email Delivery', 'certificate-generator' ) . '</h3>';
	...
	echo '<input type="email" id="cg_smtp_test_recipient" value="' . esc_attr( get_option( 'admin_email' ) ) . '" style="width:280px" placeholder="recipient@example.com">';
	...
	echo '<span id="cg_smtp_test_result" style="font-weight:500"></span>';
	echo '</div>';
	echo '<script>
    jQuery(function($){
        $("#cg_smtp_test_btn").on("click", function(){
            ...
            $.post(ajaxurl, {
                ...
                nonce:     "' . wp_create_nonce( 'cg_smtp_test_nonce' ) . '",
...
add_action(
	...
	function () {
		...
		$to   = sanitize_email( $_POST['recipient'] ?? get_option( 'admin_email' ) );
		$sent = wp_mail(
			$to,
...
// ── License-server AJAX handlers ─────────────────────────────────────────────

function cg_ajax_save_license_server(): void {
	check_ajax_referer( 'cg_license_nonce', 'nonce' );
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_send_json_error( 'Insufficient permissions' );
	}
	$url = esc_url_raw( trim( sanitize_text_field( $_POST['server_url'] ?? '' ) ) );
	$url = rtrim( $url, '/' );
	update_option( 'cg_license_server_url', $url );
	wp_send_json_success(
		array(
			'message' => __( 'License server URL saved.', 'certificate-generator' ),
			'url'     => $url,
		)
	);
}

function cg_ajax_test_license_server(): void {
	check_ajax_referer( 'cg_license_nonce', 'nonce' );
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_send_json_error( 'Insufficient permissions' );
	}
	...
	$url = rtrim( $url, '/' );
	if ( empty( $url ) ) {
		wp_send_json_error( __( 'No server URL provided.', 'certificate-generator' ) );
	}
	$response = wp_remote_get( $url . '/api/health', array( 'timeout' => 8 ) );
	if ( is_wp_error( $response ) ) {
		wp_send_json_error( $response->get_error_message() );
	}
	$code = wp_remote_retrieve_response_code( $response );
	if ( $code >= 200 && $code < 300 ) {
		wp_send_json_success( __( 'Connection successful.', 'certificate-generator' ) );
	} else {
		wp_send_json_error( sprintf( __( 'Server responded with HTTP %d.', 'certificate-generator' ), $code ) );
	}
}

function cg_ajax_verify_now(): void {
	check_ajax_referer( 'cg_license_nonce', 'nonce' );
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_send_json_error( 'Insufficient permissions' );
	}
	if ( ! class_exists( 'CG_License_Manager' ) ) {
		wp_send_json_error( 'License manager unavailable.' );
	}
	CG_License_Manager::heartbeat();
	$plan   = CG_License_Manager::get_plan_label();
	$expiry = CG_License_Manager::get_expiry();
	wp_send_json_success(
		array(
			'message' => sprintf( __( 'Verified. Current plan: %s.', 'certificate-generator' ), $plan ),
			'plan'    => $plan,
			'expiry'  => $expiry,
		)
	);
...
function certificate_generator_settings_page() {
	...
	<div class="wrap">
		...
		<?php elseif ( $active_tab == 'templates' ) : ?>
			...
			<form action="options.php" method="post">
				...
				&& wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['cg_email_test_nonce'] ) ), 'cg_email_test_send' )
					...
					$cg_test_to = sanitize_email( $_POST['cg_test_email_to'] ?? get_bloginfo( 'admin_email' ) );
				...
				<?php if ( $cg_email_msg ) : ?>
				<div class="notice notice-success is-dismissible"><p><?php echo wp_kses_post( $cg_email_msg ); ?></p></div>
			<?php endif; ?>
				...
				<table class="form-table">
					<tr>
						...
						<td>
							...
							<p class="description"><?php esc_html_e( 'Use wp_mail if your host handles outbound mail. Use SMTP to configure Gmail, SendGrid, Mailgun, etc.', 'certificate-generator' ); ?></p>
						</td>
					</tr>
				</table>
			...
			<hr>
			<h2><?php esc_html_e( 'Send Test Email', 'certificate-generator' ); ?></h2>
			<form method="post" style="display:flex;align-items:flex-end;gap:10px;flex-wrap:wrap">
				...
				<div>
					<label for="cg_test_email_to" style="display:block;margin-bottom:4px;font-weight:600"><?php esc_html_e( 'Recipient', 'certificate-generator' ); ?></label>
		...
		<?php elseif ( $active_tab == 'api' ) : ?>
			...
			<div class="cg-api-wrap">
				...
				<div class="cg-api-status active">✓ <?php esc_html_e( 'API is active and accepting requests.', 'certificate-generator' ); ?></div>
					<?php
					$table_status = '';
					if ( function_exists( 'certificate_generator_using_custom_tables' ) ) {
						$table_status = certificate_generator_using_custom_tables()
						? '<div style="margin-top:8px;font-size:12px;color:#059669;">📊 Using custom database tables (cg_students, cg_certificates)</div>'
						: '<div style="margin-top:8px;font-size:12px;color:#92400e;">📄 Using WordPress custom post types</div>';
					}
					echo $table_status;
					?>
				<?php endif; ?>
				...
				<div class="cg-endpoint">
					...
					<div class="cg-endpoint-body">
						...
						<strong style="font-size:12px;text-transform:uppercase;color:#444;"><?php esc_html_e( 'Example Request', 'certificate-generator' ); ?></strong>
						<div class="cg-code-block" id="cg-code-issue">
						<?php
				...
				<div class="cg-endpoint">
					...
					<div class="cg-endpoint-body">
						...
						<strong style="font-size:12px;text-transform:uppercase;color:#444;"><?php esc_html_e( 'Example Request', 'certificate-generator' ); ?></strong>
						<div class="cg-code-block" id="cg-code-validate">
						<?php
				...
				$using_tables = false;
				if ( function_exists( 'certificate_generator_using_custom_tables' ) ) {
					$using_tables = certificate_generator_using_custom_tables();
				}
				?>
				<?php /* Endpoint 4: certificates-by-email */ ?>
				<div class="cg-endpoint">
					<div class="cg-endpoint-head">
						<span class="cg-method get">GET</span>
						<span class="cg-endpoint-url"><?php echo esc_html( get_rest_url( null, 'certificate-generator/v1/certificates-by-email' ) ); ?></span>
						<span style="font-size:11px;background:#fef3c7;color:#92400e;padding:2px 8px;border-radius:3px;font-weight:600;">🔑 <?php esc_html_e( 'API Key required', 'certificate-generator' ); ?></span>
					</div>
					<div class="cg-endpoint-body">
						<p><?php esc_html_e( 'Look up existing certificates for a student email. Returns certificates without regenerating PDFs.', 'certificate-generator' ); ?></p>
						<strong style="font-size:12px;text-transform:uppercase;color:#444;"><?php esc_html_e( 'Parameters', 'certificate-generator' ); ?></strong>
						<table class="cg-params-table">
							<tr>
								<th><?php esc_html_e( 'Field', 'certificate-generator' ); ?></th>
							...
							<tr>
								...
								<td>query</td>
								<td><span class="cg-required"><?php esc_html_e( 'Required', 'certificate-generator' ); ?></span></td>
								<td><?php esc_html_e( 'Student email address to look up', 'certificate-generator' ); ?></td>
							</tr>
							<tr>
								<td><code>Authorization</code></td>
								<td>header</td>
						...
						<strong style="font-size:12px;text-transform:uppercase;color:#444;"><?php esc_html_e( 'Example Request', 'certificate-generator' ); ?></strong>
						<div class="cg-code-block" id="cg-code-byemail">
						<?php
						echo esc_html( "curl -X GET \\\n  \"" . get_rest_url( null, 'certificate-generator/v1/certificates-by-email' ) . "?email=student@example.com\" \\\n  -H \"Authorization: Bearer " . $example_key . '"' );
						?>
						<button class="cg-code-copy" onclick="cgCopyCode('cg-code-byemail')">Copy</button></div>
						<strong style="font-size:12px;text-transform:uppercase;color:#444;"><?php esc_html_e( 'Success Response', 'certificate-generator' ); ?></strong>
						<div class="cg-code-block">{"status": "success", "certificates": [{"title": "John Doe", "certificate_type": "Course Completion", "pdf_url": "https://...", "issued_at": "2024-01-15", "serial_number": "CG-2024-001"}], "count": 1}</div>
					</div>
				</div>
		...
		<?php elseif ( $active_tab == 'tools' ) : ?>
			...
			<!-- S4: PDF Path Verification -->
			<div style="margin-top: 30px; padding: 20px; background: #fff; border: 1px solid #ccd0d4; border-radius: 4px;">
				<h3><?php _e( 'PDF File Integrity Check', 'certificate-generator' ); ?></h3>
				<p><?php _e( 'Scan the certificates table for rows where the stored PDF path no longer exists on disk.', 'certificate-generator' ); ?></p>
		...
		(function($) {
			...
			$('#cg_pdf_check_btn').on('click', function () {
				...
				$.post(ajaxurl, {
					...
					nonce: '<?php echo wp_create_nonce( 'cg_pdf_integrity_check' ); ?>'
				}, function (r) {
					...
					if (r.success) {
						...
						$res.html('<p style="color:red">Check failed. See error log.</p>');
					}
				}).fail(function () {
					$btn.prop('disabled', false).text('<?php echo esc_js( __( 'Run Integrity Check', 'certificate-generator' ) ); ?>');
					$res.html('<p style="color:red">AJAX error.</p>');
				});
			});
		})(jQuery);
	</script>
...
function certificate_generator_is_wp_mail_smtp_active() {
	...
	if ( class_exists( 'WPMailSMTP\\Options' ) ) {
		try {
			...
			error_log( 'Certificate Generator: WP Mail SMTP Pro config error — ' . $e->getMessage() );
		}
	}
							...
							<td>
								<?php if ( $api_key ) : ?>
									<div class="api-key" style="font-family: monospace; padding: 10px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 3px; word-break: break-all;">
										<?php echo esc_html( substr( $api_key, 0, 8 ) . str_repeat( '*', max( 0, strlen( $api_key ) - 16 ) ) . substr( $api_key, -8 ) ); ?>
									</div>
								<?php else : ?>
...
add_action(
	...
	function () {
		...
		$tmpl          = $wpdb->get_row( $wpdb->prepare( "SELECT certificate_type, event_date FROM $tbl_templates WHERE id = %d", $template_id ), ARRAY_A );

		if ( ! $tmpl ) {
			wp_die( 'Template not found (ID: ' . $template_id . ').' );
		}

		$cert_type  = ! empty( $tmpl['certificate_type'] ) ? $tmpl['certificate_type'] : sanitize_text_field( $_POST['certificate_type'] ?? 'Preview' );
		$event_date = ! empty( $tmpl['event_date'] ) ? $tmpl['event_date'] : date( 'Y-m-d' );

		// Base fields from CG_Field_Schema
		$base_fields = array( 'student_name', 'school_name', 'teacher_name', 'issue_date' );

		// Include any registered extra fields for this certificate type
		if ( class_exists( 'CG_Field_Schema' ) ) {
			$extra_fields = CG_Field_Schema::get_extra_fields( $cert_type );
			$base_fields  = array_merge( $base_fields, $extra_fields );
		}
