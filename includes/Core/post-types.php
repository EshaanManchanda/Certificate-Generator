...
function cg_sync_to_dynamic_tags($post_id)
{
    ...
    if (!in_array($post_type, ['students', 'teachers', 'schools'], true)) {
        return;
    }
    ...
    foreach ($fields as $shortcode => $value) {
        ...
        $tag_name = ucwords(str_replace(['cg_', '_'], ['', ' '], $shortcode));

        $existing = $manager->get_tag_by_shortcode($shortcode);
        if ($existing && !empty($existing->id)) {
            $manager->update_tag($existing->id, ['content' => $value]);
        } else {
            $manager->create_tag([
                'tag_name' => $tag_name,
                'shortcode' => $shortcode,
...
function cg_sync_extra_fields_to_sql(int $post_id): void {
    ...
    if (!class_exists('\CertificateGenerator\Database\CustomTables')) return;
    if (!class_exists('CG_Field_Schema')) return;

    $post_type = get_post_type($post_id);
    if (!in_array($post_type, ['students', 'teachers', 'schools'], true)) return;

    global $wpdb;
    $tables = \CertificateGenerator\Database\CustomTables::instance();
    $table  = $tables->get_table($post_type);
    if (!$table || $wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s', $table)) !== $table) return;

    // Core columns that already have dedicated SQL columns — skip from JSON
    $core_keys = ['student_name', 'teacher_name', 'school_name', 'email', 'phone',
                  'certificate_type', 'issue_date', 'school_abbreviation', 'place',
                  'email_status', 'send_email', 'wp_post_id', 'status',
    ...
    foreach ($extra_slugs as $slug) {
        if (in_array($slug, $core_keys, true)) continue;
        $value = get_post_meta($post_id, $slug, true);
        if ($value !== '' && $value !== null) {
...
function render_students_extra_fields_form($post)
{
    ...
    <div class="cg-student-card" id="cg-extra-fields-card">
        ...
        <div id="cg-extra-fields-list">
            <?php foreach ($extra_fields as $slug) :
...
function save_students_data($post_id)
{
    ...
    if (isset($_POST['issue_date'])) {
        ...
        if ($_stored_issue !== null) update_post_meta($post_id, 'issue_date', $_stored_issue);
    }
...
function cg_ajax_add_extra_field()
{
    ...
    if (empty($cert_type) || empty($slug)) {
        wp_send_json_error(['message' => 'Certificate type and field name are required.']);
    }

    if (!class_exists('CG_Field_Schema')) {
        wp_send_json_error(['message' => 'Field schema not available.']);
    }
    ...
    if (!$registered) {
        ...
        if (in_array($slug, $existing, true)) {
            wp_send_json_error(['message' => "Field \"{$slug}\" is already registered."]);
        }
        wp_send_json_error(['message' => "Could not register \"{$slug}\". It may be a reserved name or the slot limit (15) has been reached."]);
    }
    ...
    wp_send_json_success(['html' => $html, 'slug' => $slug, 'label' => $label]);
}
...
function cg_certificates_duplicate_type_warning()
{
    ...
    if ($cached === false) {
        ...
        $all = get_posts(['post_type' => 'certificates', 'posts_per_page' => -1, 'fields' => 'ids']);
        $type_map = [];
        foreach ($all as $id) {
            $type = get_post_meta($id, 'certificate_type', true);
            $date = get_post_meta($id, 'event_date', true);
            if (!isset($type_map[$type])) {
                $type_map[$type] = ['count' => 0, 'has_date' => false];
            }
            $type_map[$type]['count']++;
            if ($date)
...
function cg_enqueue_cpt_admin_scripts($hook)
{
    ...
    global $post_type;
    if (in_array($post_type, ['students', 'teachers', 'schools', 'certificates'], true)) {
        wp_enqueue_media();
    }
    if ($post_type === 'certificates') {
...
{
    $screen = get_current_screen();
    if (!$screen || !in_array($screen->post_type, ['students', 'teachers', 'schools', 'certificates'], true)) {
        return;
    }
