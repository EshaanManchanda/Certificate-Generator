...
class CG_Cron_Jobs {
    public static function init() {
        add_action('cg_cleanup_qr_codes', [__CLASS__, 'cleanup_qr_codes']);
        add_action('cg_check_expiring_certificates', [__CLASS__, 'check_expiring_certificates']);
        add_action('cg_cleanup_old_certificates', [__CLASS__, 'cleanup_old_certificates']);
    ...
    public static function cleanup_old_certificates() {
        ...
        $expired_old = $wpdb->get_results($wpdb->prepare(
        ...
        foreach ($expired_old as $cert) {
            ...
            $wpdb->delete($table_name, ['id' => $cert->id]);
            $archived++;
        }
