...
function certificate_generator_send_email($post_id, $log_email = true) {
    ...
    // Get recipient information based on post type — SQL-first, CPT fallback
    $_email_sql = cg_email_get_sql_row($post_id, $post_type);
    $recipient_email = ($_email_sql !== null && !empty($_email_sql['email']))
        ? $_email_sql['email']
    ...
    // Get recipient name based on post type — SQL-first, CPT fallback
    $recipient_name = '';
    $name_field_map = ['students' => 'student_name', 'teachers' => 'teacher_name', 'schools' => 'school_name'];
    $name_key = $name_field_map[$post_type] ?? '';
    if ($name_key && $_email_sql !== null && !empty($_email_sql[$name_key])) {
        $recipient_name = $_email_sql[$name_key];
    } else {
        switch ($post_type) {
            case 'students': $recipient_name = get_post_meta($post_id, 'student_name', true); break;
            case 'teachers': $recipient_name = get_post_meta($post_id, 'teacher_name', true); break;
            case 'schools':  $recipient_name = get_post_meta($post_id, 'school_name',  true); break;
        }
    }

    // Get certificate type — SQL-first (lives in extra_fields JSON, flattened by helper)
    $certificate_type = ($_email_sql !== null && !empty($_email_sql['certificate_type']))
        ? $_email_sql['certificate_type']
    ...
    $serial_number = get_post_meta($post_id, 'certificate_serial_number', true);
    $expires_at = get_post_meta($post_id, 'certificate_expires_at', true);
    ...
    try {
        ...
        foreach ($all_certificate_ids as $index => $cert_id) {
            ...
            // Pass an empty array — generate_certificate_pdf_email() will resolve the
            // correct field list via CG_Field_Schema (including any extra fields) and
            // generate_certificate_pdf() will apply per-field visibility from the template.
            $fields = [];
...
function certificate_generator_auto_send_email($post_id, $force_send = false) {
    ...
    // Check if email was already sent to avoid duplicates — SQL-first, CPT fallback
    $auto_post_type = get_post_type($post_id);
    $auto_sql_row   = in_array($auto_post_type, ['students', 'teachers', 'schools'], true)
        ? cg_email_get_sql_row($post_id, $auto_post_type)
        : null;
    $email = ($auto_sql_row !== null && !empty($auto_sql_row['email']))
        ? $auto_sql_row['email']
        : get_post_meta($post_id, 'email', true);
