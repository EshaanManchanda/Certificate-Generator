...
function certificate_generator_get_filtered_recipients($filters = []) {
    ...
    // DEBUG: Log generated SQL and test database
    global $wpdb;
    $test_count = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->posts} WHERE post_type IN ('students', 'teachers', 'schools') AND post_status = 'publish'");
    error_log('=== SQL QUERY DEBUG ===');
    error_log('Total certificates in DB: ' . $test_count);
    error_log('Generated SQL: ' . $query);
    error_log('Post types filter: ' . print_r($filters['post_types'], true));
    error_log('Email status filter: ' . print_r($filters['email_status'], true));
    error_log('Skip already sent: ' . ($filters['skip_already_sent'] ? 'true' : 'false'));
    error_log('WHERE clauses count: ' . count($where));
    error_log('======================');
