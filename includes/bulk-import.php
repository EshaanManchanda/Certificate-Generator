    ...
    if (isset($_POST['submit_students']) && isset($_FILES['students_csv'])) {
        if (check_admin_referer('bulk_import_students_nonce', '_wpnonce_bulk_import')) {
            ...
            if (($handle = fopen($file, 'r')) !== false) {
                ...
                // Trim whitespace and normalize header keys
                $header = array_map(function ($val) {
                    return trim((string) $val); }, $header);
                $header_keys = array_map('sanitize_key', $header);
                ...
                while (($data = fgetcsv($handle, 0, ',')) !== false) {
                    ...
                    if ($post_id) {
                        ...
                        if (function_exists('update_field')) {
                            ...
                            update_field('field_certificate_type', $cert_type, $post_id);
                            ...
                            add_post_meta($post_id, 'certificate_type', $cert_type);
                            add_post_meta($post_id, 'school_abbreviation', $school_abbreviation);
                        }

                        // Save extra fields and auto-register them in the schema
                        if (!empty($extra_columns) && class_exists('CG_Field_Schema')) {
                            ...
                            foreach ($extra_columns as $slug) {
                                $value = $student_data[$slug] ?? '';
                                update_post_meta($post_id, $slug, sanitize_text_field($value));

                                // Register once per cert-type+slug combination
        ...
        if (check_admin_referer('bulk_import_certificates_nonce', '_wpnonce_bulk_import')) {
            ...
            if (($handle = fopen($file, 'r')) !== false) {
                ...
                while (($data = fgetcsv($handle, 0, ',')) !== false) {
                    ...
                    if ($post_id) {
                        // Save each recognised field — apply the same sanitisation as save_certificates_data.
                        foreach ($certificate_data as $key => $value) {
                            if ($key === 'event_date') {
                                // Validate: must be empty or a valid Y-m-d date
                                $raw = sanitize_text_field($value);
                                if ($raw === '') {
                                    update_post_meta($post_id, 'event_date', '');
                                } elseif (
                                    preg_match('/^\d{4}-\d{2}-\d{2}$/', $raw)
                                    && ($parsed = DateTime::createFromFormat('Y-m-d', $raw))
                                    && $parsed->format('Y-m-d') === $raw
                                ) {
                                    update_post_meta($post_id, 'event_date', $raw);
                                }
                                // Invalid format: skip silently (leave meta empty)
                                continue;
                            }
                            // Normalise visible flags to '1' or '0' for all field slots
                            if (preg_match('/^field_\d+_visible$/', $key)) {
                                $value = $value ? '1' : '0';
                            }
                            update_post_meta($post_id, $key, $value);
                        }

                        // Calculate template_field_count as the highest slot N with at least one non-empty value
                        ...
                        update_post_meta($post_id, 'template_field_count', $tfc ?: 3);

                        // Auto-generate the post title — mirrors save_certificates_data() in certificate-post-type.php
                        $certificate_type = sanitize_text_field($certificate_data['certificate_type']);
                        $template_url = esc_url($certificate_data['template_url']);
                        $event_date = isset($certificate_data['event_date'])
                            ? sanitize_text_field($certificate_data['event_date'])
                            : '';
                        $date_suffix = $event_date ? ' (' . $event_date . ')' : '';
                        $host = $template_url ? parse_url($template_url, PHP_URL_HOST) : null;

                        $new_title = ($certificate_type ?: 'Certificate')
                            . $date_suffix
                            . ' - '
                            . ($host ?: 'No Template URL');
