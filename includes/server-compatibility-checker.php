...
class CertificateGenerator_CompatibilityChecker {
    ...
    private function check_memory_limit() {
        ...
        $current = $this->convert_to_bytes($current_str);
        ...
        return array(
            ...
            'minimum' => $this->format_bytes($minimum),
            'recommended' => $this->format_bytes($recommended),
            'message' => $passed ?
                ($is_recommended ? "Memory limit $current_str is excellent" : "Memory limit $current_str meets minimum requirements") :
                "Memory limit $current_str is too low (minimum: " . $this->format_bytes($minimum) . " required)",
            'recommendation' => !$is_recommended ?
                "Increase memory limit to " . $this->format_bytes($recommended) . " for optimal performance" : ''
        );
    }
    ...
    private function check_max_file_size() {
        ...
        return array(
            ...
            'current' => $this->format_bytes($current),
            'minimum' => $this->format_bytes($minimum),
            'recommended' => $this->format_bytes($recommended),
            'message' => $passed ?
                ($is_recommended ? "File upload size limit is excellent" : "File upload size meets minimum requirements") :
                "File upload size " . $this->format_bytes($current) . " is too small (minimum: " . $this->format_bytes($minimum) . ")",
            'recommendation' => !$is_recommended ?
                "Increase upload limits to " . $this->format_bytes($recommended) . " for large file uploads" : ''
        );
    }
    ...
    private function check_disk_space() {
        ...
        return array(
            'passed' => $passed,
            'critical' => false,
            'current' => $available_space ? $this->format_bytes($available_space) : 'Unknown',
            'required' => $this->format_bytes($required_space),
            'message' => $passed ?
                "Sufficient disk space available" :
                "Low disk space: " . $this->format_bytes($available_space) . " available (need " . $this->format_bytes($required_space) . ")",
