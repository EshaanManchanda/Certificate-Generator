...
function generate_certificate_pdf_with_data($post_data) {
    ...
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log('Certificate Generator Debug - Query Args: ' . print_r($certificate_query->query_vars, true));
        error_log('Certificate Generator Debug - Post Data: ' . print_r($post_data, true));
    }
    ...
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log('Certificate Generator Debug - Dynamic Fields: ' . print_r($fields, true));
    }
    ...
    foreach ($fields as $index => $field) {
        ...
        if ($field_positions[$field]['align'] === "'R'" || $field_positions[$field]['align'] === '"R"') {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Certificate Generator Debug - Fixed quoted right alignment value for {$field}");
            }
        } else if ($field_positions[$field]['align'] === "'L'" || $field_positions[$field]['align'] === '"L"') {
            $field_positions[$field]['align'] = 'L';
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Certificate Generator Debug - Fixed quoted left alignment value for {$field}");
            }
        } else if ($field_positions[$field]['align'] === "'C'" || $field_positions[$field]['align'] === '"C"') {
            $field_positions[$field]['align'] = 'C';
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Certificate Generator Debug - Fixed quoted center alignment value for {$field}");
            }
        }
        ...
        if (!in_array($field_positions[$field]['align'], ['L', 'C', 'R'])) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Certificate Generator Debug - Invalid alignment value for {$field}, defaulting to Center");
            }
        }

        // Debugging: Log field positions
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Certificate Generator Debug - Field position for {$field}: " . print_r($field_positions[$field], true));
        }
    }
    ...
    foreach ($field_positions as $field => $position) {
        ...
        if ($debug_mode && defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Certificate Generator Debug - Processing field {$field}: text_width={$text_width}, field_width={$field_width}, alignment={$position['align']}");
        }
...
function generate_certificate_pdf($post_id, $fields) {
    ...
    // Start error logging
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log("Certificate Generator Debug - Starting certificate generation for post ID: $post_id");
        error_log("Certificate Generator Debug - Requested fields: " . print_r($fields, true));
    }
    ...
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log("Certificate Generator Debug - Validating template URL: {$template_url}");
    }
    $validation_result = validate_template_url($template_url);
    if ($validation_result !== true) {
    ...
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log("Certificate Generator Debug - Template URL validation successful");
    }
    ...
    foreach ($fields as $field_name) {
        ...
        if ($field_name === 'issue_date') {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Certificate Generator Debug - Formatted issue_date: " . $value);
            }
        }
    ...
    // Debug: Log post data
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log("Certificate Generator Debug - Post data for PDF: " . print_r($post_data, true));
    }
    ...
    try {
        ...
        foreach ($field_positions as $field => $position) {
            if (!isset($post_data[$field])) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("Certificate Generator Debug - Field $field exists in positions but not in post data");
                }
                continue;
            }
            ...
            if ($position['visible'] == '0') {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("Certificate Generator Debug - Skipping hidden field: {$field}");
                }
                continue;
            }
            ...
            if (empty($post_data[$field]) || !is_numeric($position['x']) || !is_numeric($position['y'])) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("Certificate Generator Debug - Invalid data or position for $field: X={$position['x']}, Y={$position['y']}, Data=" . (empty($post_data[$field]) ? 'empty' : 'present'));
                }
                continue;
            }
        ...
        if (!file_exists($upload_path)) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Certificate Generator Debug - Created upload directory: $upload_path");
            }
        }
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Certificate Generator Debug - Upload directory: " . print_r($upload_dir, true));
            error_log("Certificate Generator Debug - PDF file path: $pdf_path");
        }
        ...
        try {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Certificate Generator Debug - Template URL: " . $template_url);
                error_log("Certificate Generator Debug - PDF Path: " . $pdf_path);
                error_log("Certificate Generator Debug - Field Positions: " . print_r($field_positions, true));
                error_log("Certificate Generator Debug - Post Data: " . print_r($post_data, true));
            }
            return false;
        }
        ...
        if (!file_exists($pdf_path)) {
            ...
            error_log("Certificate Generator: " . $error_msg);
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Certificate Generator Debug - Upload directory permissions: " . (is_writable($upload_path) ? 'writable' : 'not writable'));
                error_log("Certificate Generator Debug - Free disk space: " . disk_free_space($upload_path) . " bytes");
            }
            return false;
        }
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Certificate Generator Debug - PDF generated successfully. URL: $file_url");
        }
