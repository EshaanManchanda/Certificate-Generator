 ...
 *
 * When multiple templates share the same certificate_type, the one whose
 * event_date is closest to (and preferably not after) the student's issue_date
 * is returned.  Falls back gracefully for single-template and no-date cases.
 *
 ...
 * @return WP_Post|null             Matched template post, or null if none found.
 */
/**
...
function cg_select_certificate_template($certificate_type, $issue_date_iso = '', $strict = true)
{
    ...
    if (empty($candidates)) {
        $all = get_posts(['post_type' => 'certificates', 'posts_per_page' => -1]);
        foreach ($all as $tpl) {
            $t = get_post_meta($tpl->ID, 'certificate_type', true);
    ...
    if (empty($with_date)) {
        error_log('CG Warning: Multiple templates for "' . $certificate_type
    ...
    if (!empty($issue_date_iso)) {
        ...
        if ($normalised_issue) {
            ...
            error_log('CG: No exact event_date match for certificate_type="'
    ...
    if ($normalised_issue !== '') {
        ...
        usort($with_date, function ($a, $b) use ($normalised_issue) {
...
function generate_certificate_pdf_with_data($post_data)
{
    ...
    $template_orientation = $tmpl_meta['template_orientation'][0] ?? 'portrait';
    ...
    // Dynamically determine fields from post_data (exclude certificate_type and internal _ keys).
    // array_values() re-indexes to 0, 1, 2 … so that $field_num + 1 correctly maps to
    // field_1_*, field_2_*, field_3_* meta keys stored in the certificate template.
    $fields = array_values(array_filter(
        array_keys($post_data),
        fn($f) => $f !== 'certificate_type' && strpos($f, '_') !== 0
    ));

    cg_debug_log('Dynamic Fields: ' . print_r($fields, true));

    // Fetch field positions dynamically from the certificate template.
    // $tmpl_meta was already fetched above — no extra queries needed here.
    $field_positions = [];
    foreach ($fields as $field_num => $field) {
        $field_key = $field_num + 1; // 1-indexed: field_1_*, field_2_*, field_3_*, …
        $field_positions[$field] = [
            ...
            'visible' => $tmpl_meta["field_{$field_key}_visible"][0]     ?? '0',
        ];
...
function generate_certificate_pdf($post_id, $fields)
{
    ...
    // Start error logging
    cg_debug_log("Starting certificate generation for post ID: $post_id");
    ...
    // ── Usage-limit gate ─────────────────────────────────────────────────
    // Allows admin-usage-tracker.php (and any third-party code) to block
    // generation when the monthly limit has been reached.
    ...
    // Get student's issue_date for date-based template matching
    ...
    // Fetch field positions dynamically from the certificate template.
    // $tmpl_meta was already fetched above — no extra queries needed here.
    // array_values() ensures the index is always 0-based even if the caller passes
    // an associative or non-sequential array.
    $fields = array_values($fields);
    $field_positions = [];
    $missing_positions = [];

    foreach ($fields as $index => $field) {
        $field_key = $index + 1; // 1-indexed: field_1_*, field_2_*, field_3_*, …
        $field_positions[$field] = [
            'x'       => $tmpl_meta["field_{$field_key}_position_x"][0]  ?? '',
            'y'       => $tmpl_meta["field_{$field_key}_position_y"][0]  ?? '',
            'width'   => $tmpl_meta["field_{$field_key}_width"][0]       ?? '100',
    ...
    if (isset($_GET['teacher_email'])) {
        ...
        if ($query->have_posts()) {
            while ($query->have_posts()) {
                ...
                if (!cg_resolve_student_template($post_id)) {
                    error_log('Certificate Generator: No matching template for teacher post ' . $post_id
    ...
    if (isset($_GET['school_name']) && isset($_GET['place'])) {
        ...
        if ($query->have_posts()) {
            ...
            while ($query->have_posts()) {
                ...
                $current_cert_type = get_post_meta($post_id, 'certificate_type', true);
                ...
                // ── Double-match pre-check: certificate_type + issue_date → event_date ──
                if (!cg_resolve_student_template($post_id)) {
                    error_log('Certificate Generator: No matching template for school post ' . $post_id
...
function scs_student_search_shortcode()
{
    if (isset($_GET['student_email'])) {
        ...
        if ($query->have_posts()) {
            ...
            $certificates = [];
            $processed_certificates = []; // Track processed certificates to avoid duplicates
            $no_template_students = [];   // students whose certificate_type/issue_date found no template
            ...
            while ($query->have_posts()) {
                ...
                // ── Double-match pre-check: certificate_type + issue_date → event_date ──
                // Verify a matching template exists BEFORE attempting expensive PDF generation.
                if (!cg_resolve_student_template($post_id)) {
                    $no_template_students[] = [
                        'name' => get_post_meta($post_id, 'student_name', true) ?: get_the_title(),
                        'certificate_type' => get_post_meta($post_id, 'certificate_type', true),
                        'issue_date' => get_post_meta($post_id, 'issue_date', true),
                    ];
                    error_log('Certificate Generator: No matching template for student post ' . $post_id
                        . ' (type="' . get_post_meta($post_id, 'certificate_type', true) . '"'
                        . ', issue_date="' . get_post_meta($post_id, 'issue_date', true) . '")');
                    continue; // skip PDF generation — no template available
                }
                ...
                if ($file_url) {
                    ...
                    // Add to certificates array - don't skip based on file path as different 
                    // student posts might legitimately map to the same cached/generated PDF path 
                    // if caching is overly aggressive, but we still want to show all their records.
                    // We only deduplicate by post_id.
