				...
				<div class="cg-chart-wrap">
					...
					<?php else : ?>
						...
						foreach ( $chart_labels as $i => $lbl ) :
							$val      = $chart_values[ $i ];
							$pct      = (int) round( ( $val / $chart_max ) * 100 );
						...
						<div class="cg-bar-row">
							...
							<div class="cg-bar-track">
								<div class="cg-bar-fill" style="width:<?php echo esc_attr( (string) $pct ); ?>%">
									<?php
									if ( $val > 0 ) :
										?>
										<span class="cg-bar-val"><?php echo esc_html( (string) $val ); ?></span><?php endif; ?>
								</div>
							</div>
						</div>
						<?php endforeach; ?>
				...
				<div class="cg-chart-wrap">
					...
					<?php if ( empty( $top_types ) ) : ?>
						...
						foreach ( $top_types as $t ) :
							$pct = $type_max > 0 ? (int) round( ( $t->cnt / $type_max ) * 100 ) : 0;
							?>
					<div class="cg-bar-row" style="margin-bottom:10px">
						<div style="flex:1">
							...
							<div class="cg-bar-track">
								<div class="cg-bar-fill" style="width:<?php echo esc_attr( (string) $pct ); ?>%;background:#27ae60">
									<?php
									if ( (int) $t->cnt > 0 ) :
										?>
										<span class="cg-bar-val"><?php echo esc_html( $t->cnt ); ?></span><?php endif; ?>
								</div>
							</div>
						</div>
					</div>
