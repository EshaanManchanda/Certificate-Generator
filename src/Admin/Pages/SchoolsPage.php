...
class SchoolsPage {
    ...
    public function register(): void {
        add_submenu_page('cg-dashboard', 'Schools', 'Schools', 'manage_options', $this->slug, [$this, 'render_list']);
        add_submenu_page('', 'Add / Edit School', '', 'manage_options', $this->edit_slug, [$this, 'render_edit']);
    }
    ...
    public function render_list(): void {
        ...
        if (!empty($_POST['cg_delete_school_nonce'])
            ...
            $ids = array_map('absint', (array) ($_POST['school_ids'] ?? []));
            foreach ($ids as $id) {
        ...
        $per_page = 20;
        $allowed  = ['school_name','email','city','state','certificate_type','issue_date','created_at'];
        ...
        if ($city_f) { $where .= ' AND city = %s'; $params[] = $city_f; }
        ...
        <div class="wrap">
            ...
            <form method="get" style="margin:15px 0;">
                ...
                <select name="city_filter">
                    ...
                    <?php foreach ($cities as $c): ?><option value="<?php echo esc_attr($c); ?>" <?php selected($city_f, $c); ?>><?php echo esc_html($c); ?></option><?php endforeach; ?>
                </select>
                ...
                <?php if ($search || $city_f || $cert_f): ?><a href="<?php echo esc_url($list_url); ?>" class="button">Clear</a><?php endif; ?>
            </form>
            ...
            <form method="post" id="cg-schools-form">
                ...
                <table class="wp-list-table widefat fixed striped">
                    <thead><tr>
                        ...
                        <?php
                        $cols = ['school_name'=>'Name','email'=>'Email','city'=>'City','state'=>'State','principal_name'=>'Principal','certificate_type'=>'Cert. Type','issue_date'=>'Issue Date','status'=>'Status'];
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                &nbsp;|&nbsp;
                                <a href="<?php echo esc_url(wp_nonce_url(add_query_arg(['page'=>$this->slug,'action'=>'delete','id'=>$row_id], admin_url('admin.php')), 'cg_delete_school_single_'.$row_id)); ?>"
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST'
            ...
            if (empty($errors)) {
                ...
                $data = [
                    ...
                    'status'           => in_array($_POST['status'] ?? '', ['active','inactive'], true) ? sanitize_key($_POST['status']) : 'active',
                ...
                if ($id && $row) {
                    ...
                    $message = 'School updated.'; $row = array_merge($row, $data); $extra = $new_extra;
                } else {
