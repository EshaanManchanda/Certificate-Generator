...
class TeachersPage {
    ...
    public function register(): void {
        add_submenu_page('cg-dashboard', 'Teachers', 'Teachers', 'manage_options', $this->slug, [$this, 'render_list']);
        add_submenu_page('', 'Add / Edit Teacher', '', 'manage_options', $this->edit_slug, [$this, 'render_edit']);
    }
    ...
    public function render_list(): void {
        ...
        if (!empty($_POST['cg_delete_teacher_nonce'])
            ...
            $ids = array_map('absint', (array) ($_POST['teacher_ids'] ?? []));
            foreach ($ids as $id) {
        ...
        $per_page = 20;
        $allowed  = ['teacher_name','email','school_name','department','certificate_type','issue_date','created_at'];
        ...
        <div class="wrap">
            ...
            <form method="get" style="margin:15px 0;">
                ...
                <select name="school_filter">
                    ...
                    <?php foreach ($schools as $s): ?><option value="<?php echo esc_attr($s); ?>" <?php selected($school_f, $s); ?>><?php echo esc_html($s); ?></option><?php endforeach; ?>
                </select>
                <select name="cert_filter" style="margin-left:5px;">
                    <option value="">— All Certificate Types —</option>
                    <?php foreach ($cert_types as $c): ?><option value="<?php echo esc_attr($c); ?>" <?php selected($cert_f, $c); ?>><?php echo esc_html($c); ?></option><?php endforeach; ?>
                </select>
                <button type="submit" class="button">Filter</button>
                <?php if ($search || $school_f || $cert_f): ?><a href="<?php echo esc_url($list_url); ?>" class="button">Clear</a><?php endif; ?>
            </form>
            ...
            <form method="post" id="cg-teachers-form">
                ...
                <table class="wp-list-table widefat fixed striped">
                    <thead><tr>
                        ...
                        <?php
                        $cols = ['teacher_name'=>'Name','email'=>'Email','school_name'=>'School','department'=>'Department','certificate_type'=>'Cert. Type','issue_date'=>'Issue Date','serial_number'=>'Serial #','status'=>'Status'];
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                &nbsp;|&nbsp;
                                <a href="<?php echo esc_url(wp_nonce_url(add_query_arg(['page'=>$this->slug,'action'=>'delete','id'=>$row_id], admin_url('admin.php')), 'cg_delete_teacher_single_'.$row_id)); ?>"
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST'
            ...
            if (empty($errors)) {
                ...
                foreach ($ex_keys as $i => $k) {
                    $k = sanitize_key($k); $v = sanitize_text_field($ex_vals[$i] ?? '');
                ...
                $data = [
                    ...
                    'status'           => in_array($_POST['status'] ?? '', ['active','inactive','retired'], true) ? sanitize_key($_POST['status']) : 'active',
                ...
                if ($id && $row) {
                    ...
                    $message = 'Teacher updated.'; $row = array_merge($row, $data); $extra = $new_extra;
                } else {
                    $data['created_at'] = current_time('mysql');
                    $wpdb->insert($table, $data); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                    $id = (int) $wpdb->insert_id; $row = array_merge($data, ['id' => $id]); $extra = $new_extra;
                ...
                <table class="form-table">
                    ...
                    <tr><th><label for="status">Status</label></th>
                        <td><select id="status" name="status">
                            <?php foreach (['active','inactive','retired'] as $s): ?>
