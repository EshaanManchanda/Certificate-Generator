...
class DataMigration {
	...
	public function migrate_schools(): array {
		...
		return array(
			'migrated' => $migrated,
			'message'  => "Migrated $migrated schools",
		);
	}
	...
	public function migrate_students(): array {
		...
		return array(
			'migrated' => $migrated,
			'message'  => "Migrated $migrated students",
		);
	}
	...
	public function migrate_teachers(): array {
		...
		return array(
			'migrated' => $migrated,
			'message'  => "Migrated $migrated teachers",
		);
	}
	...
	public function migrate_templates(): array {
		...
		return array(
			'migrated' => $migrated,
			'message'  => "Migrated $migrated templates",
		);
	}
	...
	public function migrate_certificates(): array {
		...
		if ( ! $old_exists ) {
			return array(
				'migrated' => 0,
				'message'  => 'No old certificate table found',
			);
		}
		...
		return array(
			'migrated' => $migrated,
			'message'  => "Migrated $migrated certificates",
		);
	}
	...
	public function sync_student( int $post_id ): void {
		...
		$core_keys    = array( 'student_name', 'email', 'phone', 'school_name', 'status', 'wp_post_id', 'id', 'created_at', 'updated_at', 'extra_fields' );
		$extra_fields = array();
		foreach ( $meta as $key => $values ) {
	...
	public function sync_teacher( int $post_id ): void {
		$post = get_post( $post_id );
		if ( ! $post || $post->post_type !== 'teachers' ) {
			return;
		}
		...
		$core_keys    = array( 'teacher_name', 'email', 'phone', 'school_name', 'department', 'status', 'wp_post_id', 'id', 'created_at', 'updated_at', 'extra_fields' );
		$extra_fields = array();
		foreach ( $meta as $key => $values ) {
			$clean_key = preg_replace( '/^field_/', '', $key );
			if ( ! in_array( $clean_key, $core_keys, true ) && ! in_array( $key, $core_keys, true ) ) {
	...
	public function sync_school( int $post_id ): void {
		$post = get_post( $post_id );
		if ( ! $post || $post->post_type !== 'schools' ) {
			return;
		}
		...
		$core_keys    = array( 'school_name', 'address', 'city', 'state', 'country', 'postal_code', 'phone', 'email', 'website', 'principal_name', 'status', 'wp_post_id', 'id', 'created_at', 'updated_at', 'extra_fields' );
		$extra_fields = array();
		foreach ( $meta as $key => $values ) {
			$clean_key = preg_replace( '/^field_/', '', $key );
			if ( ! in_array( $clean_key, $core_keys, true ) && ! in_array( $key, $core_keys, true ) ) {
	...
	public function sync_template( int $post_id ): void {
		$post = get_post( $post_id );
		if ( ! $post || $post->post_type !== 'certificates' ) {
			return;
		}
