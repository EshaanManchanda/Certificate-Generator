...
class BulkIO {
		...
		foreach ( $rows as $index => $row ) {
			try {
				...
				if ( ! empty( $data['email'] ) ) {
					...
					if ( $existing ) {
						// Update existing
						$GLOBALS['wpdb']->update( $table, $prepared, array( 'id' => $existing ) );
					} else {
						// Insert new
	 ...
	 */
	public function export_students( array $filters = array() ): array {
		global $wpdb;
		$table = $this->tables->get_table( 'students' );
		if ( empty( $table ) ) {
			return array(
		...
		$core_headers = array( 'id', 'student_name', 'email', 'phone', 'school_name', 'status' );
	 ...
	 */
	public function export_teachers( array $filters = array() ): array {
		global $wpdb;
		$table = $this->tables->get_table( 'teachers' );
		if ( empty( $table ) ) {
			return array(
		...
		$core_headers = array( 'id', 'teacher_name', 'email', 'phone', 'school_name', 'department', 'status' );
		$headers      = array_merge( $core_headers, $extra_keys );
	 ...
	 */
	public function export_schools( array $filters = array() ): array {
		global $wpdb;
		$table = $this->tables->get_table( 'schools' );
		if ( empty( $table ) ) {
			return array(
		...
		$core_headers = array( 'id', 'school_name', 'address', 'city', 'state', 'country', 'postal_code', 'phone', 'email', 'website', 'principal_name', 'status' );
		$headers      = array_merge( $core_headers, $extra_keys );
