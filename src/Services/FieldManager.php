...
class FieldManager {
    ...
    private const CORE_FIELDS = [
        'students' => ['student_name', 'email', 'phone', 'school_id', 'school_name', 'enrollment_date', 'graduation_date', 'status', 'wp_post_id', 'id', 'created_at', 'updated_at', 'extra_fields'],
        'teachers' => ['teacher_name', 'email', 'phone', 'school_id', 'school_name', 'department', 'hire_date', 'status', 'wp_post_id', 'id', 'created_at', 'updated_at', 'extra_fields'],
        'schools'  => ['school_name', 'address', 'city', 'state', 'country', 'postal_code', 'phone', 'email', 'website', 'principal_name', 'status', 'wp_post_id', 'id', 'created_at', 'updated_at', 'extra_fields'],
    ];
    ...
    public static function split(string $entity_type, array $flat_data): array {
        $core_keys = self::CORE_FIELDS[$entity_type] ?? [];
        $core = [];
        $extra = [];
    ...
    public static function merge(array $core_data, ?string $extra_json = null): array {
        $extra = !empty($extra_json) ? json_decode($extra_json, true) : [];
    ...
    public static function query_by_extra_field(string $entity_type, string $field_key, string $value, string $comparison = '='): array {
        ...
        return array_map([self::class, 'prepare_for_display'], $rows);
    }
}
